    ...
    $.fn.rvc = function(obj) {
        return this.each(function() {
            ...
            $(window).on('load resize', function() {
                ...
                if (onload) { rvcw_ul.css('left', -(liw + sb) * (size / 3));
                    onload = false; }
            })

            function move_left() { next_b = true;
                rvcw_ul.stop().animate({ left: -iCurr * (liw + sb) }, time, function() { iCurr++;
                    next_b = false; if (iCurr == (size / 3) * 2 + 1) { rvcw_ul.css({ left: -(liw + sb) * (size / 3) });
                        iCurr = size / 3 + 1; } }); }
            ...
            function move_right() { prv_b = true;
                rvcw_ul.stop().animate({ left: -iCurr * (liw + sb) }, time, function() { iCurr--;
                    prv_b = false; if (iCurr == -1) { rvcw_ul.css({ left: -(liw + sb) * (size / 3) });
                        iCurr = size / 3 - 1; } }); }
            ...
            function touchPos(e) {
                var touches = e.changedTouches,
                    l = touches.length,
                    touch, tagX, tagY;
                for (var i = 0; i < l; i++) { touch = touches[i];
                    tagX = touch.clientX;
                    tagY = touch.clientY; }
                oPosition.x = tagX;
                oPosition.y = tagY;
            ...
            function touchStartFunc(e) { touchPos(e);
                startX = oPosition.x;
                startY = oPosition.y;
                temPos = rvcw_ul.position().left;
                st = parseInt(rvcw_ul.css('left')); }

            function touchMoveFunc(e) { touchPos(e); var moveX = oPosition.x - startX; var moveY = oPosition.y - startY; var stm = st + moveX;
            ...
            function touchEndFunc(e) {
                ...
                if (Math.abs(moveY) < Math.abs(moveX)) {
                    if (moveX > 0) { move_right(); } else { move_left(); }
                }
            }
            if (system.win || system.mac || system.xll) {
                rvcw_ul.mousedown(function(event) {
                    ...
                    $(document).mousemove(function(event) { var event = event || window.event;
                        ...
                        iul = iL + ol;
                        rvcw_ul.css('left', iul); return false })
                    $(document).mouseup(function() { $(document).unbind('mousemove');
                        $(document).unbind('mouseup'); if (iL > 0) { move_right(); } else { move_left(); } })
                    return false
                })
            } else { rvcw_ul.get(0).addEventListener('touchstart', touchStartFunc, false);
